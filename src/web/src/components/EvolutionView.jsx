              ...
              <CardContent>
                {predecessors.length > 0 ? (
                  <div className="space-y-4">
                    {predecessors.map((d) => <DigimonCard key={d.id} digimon={d} onClick={onDigimonSelect} onImageClick={onImagePreview} />)}
                  </div>
                ) : <p className="text-gray-500 dark:text-gray-400 italic text-center py-8">Nenhuma pré-evolução encontrada</p>}
              </CardContent>
            </Card>
            ...
            <Card className="digimon-card">
              ...
              <CardContent>
                {successors.length > 0 ? (
                  <div className="space-y-4">
                    {successors.map((d) => (
                      <DigimonCard key={d.id} digimon={d} onClick={onDigimonSelect} onImageClick={onImagePreview}>
                        <div className="pt-3 mt-3 border-t border-border/50">
                          <EvolutionRequirements requirements={d.requirements} />
                        </div>
                      </DigimonCard>
                    ))}
                  </div>
                ) : <p className="text-gray-500 dark:text-gray-400 italic text-center py-8">Nenhuma evolução encontrada</p>}
              </CardContent>
            </Card>
          </div>
        </TabsContent>
